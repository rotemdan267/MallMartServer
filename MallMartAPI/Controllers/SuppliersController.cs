...
namespace MallMartAPI.Controllers
{
    ...
    {
        private IGenericRepository<Supplier> repo;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Supplier item)
        {
            var newItem = await repo.Insert(item);
            return CreatedAtAction(nameof(GetById), new { id = newItem.Id, controller = "suppliers" }, newItem.Id);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromRoute] int id, [FromBody] Supplier item)
        {
