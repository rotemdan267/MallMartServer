...
namespace MallMartAPI.Controllers
{
    ...
    {
        private IGenericRepository<Employee> repo;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Employee item)
        {
            var newItem = await repo.Insert(item);
            return CreatedAtAction(nameof(GetById), new { id = newItem.Id, controller = "employees" }, newItem.Id);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromRoute] int id, [FromBody] Employee item)
        {
